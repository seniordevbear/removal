...
def fill_input_data(page, dataRow) : 
    ...
    span_element = page.ele("tag:span@@text()=Delete My Record")
    span_element.click()

    sleep(1)

    fName_input = page.ele("tag:input@@id=first-name")
    fName_input.click()
    print("typing the first name...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(fName_input, fName)

    lName_input = page.ele("tag:input@@id=last-name")
    lName_input.click()
    print("typing the last name...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(lName_input, lName)    

    address_input = page.ele("tag:input@@id=street-address")
    address_input.click()
    sleep(random.uniform(0.1,0.5))
    ...
    birth_month_input = page.ele("tag:input@@id=birth_month")
    birth_month_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(birth_month_input, str(dataRow["Birth Month"]))

    birth_year_input = page.ele("tag:input@@id=birth_year")
    birth_year_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(birth_year_input, str(dataRow["Birth Year"]))

    span_checkbox_element = page.ele("tag:span@@text():By checking off this box")
    span_checkbox_element.click()

    fullName_input = page.ele("tag:input@@id=signature")
    fullName_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(fullName_input, dataRow["Name"])

def leadjigcom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\LeadjigCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://d3b1yz3rpcl3c6.cloudfront.net/")
        ...
        sleep(5)
        submit_button = page.ele("tag:button@@type=submit@@text()=Submit")
        submit_button.click()
        sleep(2)
