...
def fill_input_data(page, dataRow) : 
    ...
    request_man = iframe_container.ele("tag:span@@text()= Do Not Sell or Share My Information ")
    request_man.click()

    request_type = iframe_container.ele("tag:span@@text()= Consumer ")
    request_type.click()
...
def eltorocom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        ...
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\EltoroCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://eltoro.com/do-not-sell-my-personal-information/")

        sleep(random.uniform(1, 2))

        page.wait.ele_displayed("tag:div@@class=ot-form-wrapper")
        div_container = page.ele("tag:div@@class=ot-form-wrapper")
        print(div_container)
