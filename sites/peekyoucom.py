...
def fill_input_data(page, dataRow) : 
    ...
    email_input = page.ele("tag:input@@id=email_from")
    email_input.click()
    print("typing the email...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(email_input, email_str)

    email_confirm_input = page.ele("tag:input@@id=email_v")
    ...
    checkbox1 = page.ele("tag:input@@id=check1")
    checkbox1.click()
    checkbox3 = page.ele("tag:input@@id=check3")
    checkbox3.click()
    checkbox4 = page.ele("tag:input@@id=check4")
    checkbox4.click()
    checkbox5 = page.ele("tag:input@@id=check5")
    checkbox5.click()

def peekyoucom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\PeekyouCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://www.peekyou.com/about/contact/ccpa_optout/do_not_sell/")

        fill_input_data(page, dataRow)

        apiKey = os.getenv("TWOCAPTCHA_API_KEY", "")
        site_key = "11029fba-9634-4506-a446-b49263505a20"
        ...
        form_container = page.ele("tag:form@@id=commentForm")

        g_textarea = form_container.ele("tag:textarea@@name=g-recaptcha-response")
        # g_textarea.set.attr("value", Code)
        g_textarea.set.innerHTML(Code)

        h_textarea = form_container.ele("tag:textarea@@name=h-captcha-response")
        # h_textarea.set.attr("value", Code)
        h_textarea.set.innerHTML(Code)

        submit_button = form_container.ele("tag:button@@id=genericSubmit")
        print(submit_button)
        submit_button.click()
