...
def fill_input_data(page, dataRow) : 
    ...
    fName_input = page.ele("tag:input@@name=fname")
    fName_input.click()
    print("typing the first name...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(fName_input, fName)

    lName_input = page.ele("tag:input@@name=lname")
    lName_input.click()
    ...
    state_element = page.ele("tag:select@@name=nf-field-17")
    ...
    request_input = page.ele("tag:input@@name=nf-field-21")
    request_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(request_input, "10")

    comment_input = page.ele("tag:textarea@@name=nf-field-22")
    comment_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(comment_input, "I wanto to remove my info.")

def kingmarketinggroupcom(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\KingMarketingGroupCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://kingmarketinggroup.com/do-not-sell-my-personal-information/")
        ...
        page.wait.ele_displayed("tag:input@@value=Submit")
        submit_button = page.ele("tag:input@@value=Submit")
        submit_button.click()

        page.wait.ele_displayed("tag:p@@text()=Your form has been successfully submitted.")
