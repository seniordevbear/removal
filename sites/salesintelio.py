...
def fill_input_data(page, dataRow) : 
    ...
    state_element = page.ele("tag:select@@name=request_type")
...
def salesintelio(dataRow, website_name, in_user_email, run_mode) : 
    ...
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\SalesintelIO_" + fName + "-" + lName + ".png"
        ...
        page.get("https://salesintel.io/opt-out/")
