...
def fill_input_data(page, dataRow) : 
    ...
    firstName_input = page.ele("tag:input@@name=fname")
    firstName_input.click()
    print("typing the first name...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(firstName_input, fName)
    
    lastName_input = page.ele("tag:input@@name=lname")
    lastName_input.click()
    print("typing the last name...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(lastName_input, lName)
    ...
    birth_day_select = page.ele("tag:select@@name=birth_day")
    sleep(random.uniform(0.1,0.5))
    birth_day_select.select.by_text(str(dataRow["Birth Day"]))
    
    
    birth_month_select = page.ele("tag:select@@name=birth_month")
    sleep(random.uniform(0.1,0.5))
    birth_month_select.select.by_text(str(dataRow["Birth Month"]))
    
    birth_year_select = page.ele("tag:select@@name=birth_year")
    sleep(random.uniform(0.1,0.5))
...
def golookupcom(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        ...
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\GolookupCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://golookup.com/support/optout")
        ...
        try :
            site_key = "6Ld46RoUAAAAAMQ067-fD5nfBiakE-lzajBaRqFa"
            site_url = "https://golookup.com/support/optout"
        ...
        form_container = page.ele("tag:form@@id=optout-form")
