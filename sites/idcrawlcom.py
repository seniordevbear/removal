...
def fill_input_data(page, dataRow) : 
    ...
    name_url = "https://www.idcrawl.com/" + fName + "-" + lName

    url_input = page.ele("tag:input@@id=gl-contact-url")
    url_input.click()
    sleep(random.uniform(0.1, 0.5))
    _human_type2(url_input, name_url)

    email_str = generate_email(dataRow["Name"])
    email_input = page.ele("tag:input@@id=gl-contact-email")
    email_input.clear()
    email_input.click()
...
def idcrawlcom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        ...
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\IdcrawlCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://www.idcrawl.com/remove-my-information")
        ...
        submit_btn = page.ele("tag:input@@value=Submit")
        submit_btn.click()
