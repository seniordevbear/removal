...
def fill_input_data(page, dataRow) : 
    ...
    state_input = page.ele("tag:input@@id=formField21DSARElement")
    state_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(state_input, dataRow["State"])
    sleep(random.uniform(0.1,0.5))
    page.actions.key_down("DOWN")
    sleep(random.uniform(0.1,0.5))
...
def alliantdatacom(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        ...
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\AlliantDataCom_" + fName + "-" + lName + ".png"
        ...
        # username = 'nlmjhufx'
        # password = 'g243d58jwpjk'
        # endpoint = ip_address_arr[random_number]
        # port = port_arr[random_number]

        # print(endpoint + ":" + port)
        # http://sp1vj8y5du:o2ajgcB~6lLHMc22ep@isp.smartproxy.com:10001
        ...
        page.get("https://privacyportal-cdn.onetrust.com/dsarwebform/591ac1c1-3a1e-496f-9e43-ff4afb5fef85/2b52262e-8ada-4725-b86e-e4b960336f96.html")
