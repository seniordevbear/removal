...
def fill_input_data(page, dataRow) : 
    ...
    delete_label = page.ele("tag:label@@for=right_to_delete")
    delete_label.click()

    fName_input = page.ele("tag:input@@placeholder=Enter First Name")
    fName_input.click()
    print("typing the first name...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(fName_input, fName)

    lName_input = page.ele("tag:input@@placeholder=Enter last Name")
    lName_input.click()
    print("typing the last name...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(lName_input, lName)

    address_input = page.ele("tag:input@@placeholder=Enter Address")
    address_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(address_input, dataRow["Address"])

    city_input = page.ele("tag:input@@placeholder=Enter City")
    city_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(city_input, dataRow["City"])

    state_select = page.ele("tag:select@@type=text")
    state_select.select.by_value(usaStateDictionary[dataRow["State"]])

    zip_input = page.ele("tag:input@@placeholder=Enter Zipcode")
    zip_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(zip_input, str(dataRow["Zipcode"]))

    phone_input = page.ele("tag:input@@placeholder=Enter Phone")
    phone_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(phone_input, generate_phone_number())

    email_input = page.ele("tag:input@@placeholder=Enter E-mail")
    email_input.click()
    print("typing the email...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(email_input, generate_email(dataRow["Name"]))

    checkbox_element = page.ele("tag:input@@name=attest")
    checkbox_element.click()
    apiKey = os.getenv("TWOCAPTCHA_API_KEY", "")
    solver = TwoCaptcha(apiKey)
    print("Captcha is solving...")
    try :
        site_key = "6LdbNcUqAAAAAIbmw0LIHdk0ySKYrQtSgGrlo8l4"
        site_url = "https://www.apollointeractive.com/data-rights.php"
    ...
    submit_button = page.ele("tag:button@@name=datarights_submit")
    submit_button.click()

def apollointeractivecom(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        ...
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\ApolloInteractiveCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://www.apollointeractive.com/data-rights.php")
