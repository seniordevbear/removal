...
def fill_input_data(page, dataRow) : 
    ...
    request_type = page.ele("tag:select@@name=RequestType")
    request_type.select.by_index(5)

    fName_input = page.ele("tag:input@@id=input-134")
    fName_input.click()
    print("typing the first name...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(fName_input, fName)

    lName_input = page.ele("tag:input@@id=input-144")
    lName_input.click()
    print("typing the last name...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(lName_input, lName)

    birth_date = str(dataRow["Birth Day"]) + " " + monthDictionary[str(dataRow["Birth Month"])] + " " + str(dataRow["Birth Year"])
    birth_input = page.ele("tag:input@@id=input-154")
    birth_input.input(birth_date)

    phone_input = page.ele("tag:input@@id=input-165")
    phone_input.click()
    print("typing the phone...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(phone_input, generate_phone_number())

    email_input = page.ele("tag:input@@id=input-174")
    email_input.click()
    print("typing the email...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(email_input, generate_email(dataRow["Name"]))

    country_input = page.ele("tag:input@@id=combobox-input-184")
    country_input.set.attr("value", "United States")

    address_input = page.ele("tag:textarea@@id=input-187")
    address_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(address_input, dataRow["Address"])

    city_input = page.ele("tag:input@@id=input-189")
    city_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(city_input, dataRow["City"])

    state_input = page.ele("tag:input@@id=combobox-input-192")
    state_input.clear()
    state_input.set.attr("value", dataRow["State"])
    
    zip_input = page.ele("tag:input@@id=input-196")
    zip_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(zip_input, str(dataRow["Zipcode"]))

    detail_textarea = page.ele("tag:textarea@@id=input-202")
    detail_textarea.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(detail_textarea, "I want to remove my info.")

def reedelsevierinc3mysitecom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\Reedelsevierinc3mysiteCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://reedelsevierinc3.my.site.com/webform/s/")
