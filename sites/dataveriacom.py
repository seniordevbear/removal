...
def dataveriacom(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        ...
        screenshot_save_path = screentShotDir + "\DataveriaCom_" + fName + "-" + lName + ".png"
        ...
        search_button = page.ele("tag:button@@value=Submit")
        search_button.click()
        ...
        if len(people_list) >= 2 :
            profile_url = people_list[1].ele("tag:a@@text()=View Details").attr("href")
            print(profile_url)
            page.get("https://dataveria.com/opt-out/privacy")
            sleep(1)
            ...
            try :
                site_key = "0x4AAAAAAAB9P4C_C3tSzsxi"
                url = "https://dataveria.com/opt-out/privacy"
            ...
            captcha_div = page.ele("tag:div@@id=g-recaptcha")
            print(captcha_div)
            token_input = captcha_div.ele('tag:input@@name=cf-turnstile-response')
            print(token_input)
            print(token_input)
            token_input.set.attr("value", code)
            sleep(1)
            g_captcha_input = captcha_div.ele("tag:input@@name=g-recaptcha-response")
            print(g_captcha_input)                
            g_captcha_input.set.attr("value", code)
