...
def fastpeoplesearchcom(dataRow, website_name, in_user_email, run_mode):
    ...
    try :
        ...
        try :
            site_key = "6LcwepYeAAAAAGdPe02cGfaSa5TfzgsoRcy9Vwlc"
            site_url = "https://www.fastpeoplesearch.com/removal"
        ...
        sleep(1)
        free_search_btn = page.ele("tag:button@@text()= Free Search")
        free_search_btn.click()
        
        sleep(1)

        people_list_container = page.ele("tag:div@@class=people-list")
        people_list = people_list_container.eles("tag:div@@class=card")
        ...
        view_free_details = page.eles("tag:a@@text()=View Free Details")

        if len(view_free_details) > 0 :
            view_free_details[0].click()
            page.wait.ele_displayed("tag:a@@title=Remove this record")
