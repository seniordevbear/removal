...
def fill_input_data(page, dataRow) : 
    ...
    state_select = page.ele("tag:select@@id=sublocation")
    
    state_select.select.by_text(dataRow["State"] + " (" + usaStateDictionary_1[dataRow["State"]] + ")")

    sleep(random.uniform(0.1,0.5))
    request_type_select = page.ele("tag:select@@id=dataRequestType")
    request_type_select.select.by_text("Deletion Request")

    sleep(random.uniform(0.1,0.5))
    checkbox_element = page.ele("tag:input@@id=confirmation")
...
def realtyhopcom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\RealtyhopCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://privacy.realtyhop.com/privacy-request")
