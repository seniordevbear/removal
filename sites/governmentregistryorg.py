...
def fill_input_data(page, dataRow) : 
    ...
    fName_input = main_container.ele("tag:input@@id=firstName")
    fName_input.click()
    print("typing the first name...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(fName_input, fName)

    lName_input = main_container.ele("tag:input@@id=lastName")
    lName_input.click()
    print("typing the last name...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(lName_input, lName)

    city_input = main_container.ele("tag:input@@id=city")
    city_input.click()
    sleep(random.uniform(0.1,0.5))
...
def governmentregistryorg(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        ...
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\GovernmentRegistryOrg_" + fName + "-" + lName + ".png"
        ...
        page.get("https://www.governmentregistry.org/opt-out")
