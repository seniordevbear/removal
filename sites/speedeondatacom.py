...
def speedeondatacom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\SpeedeonDataCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://optout.speedeondata.com/")

        sleep(1)

        delete_input = page.ele("tag:input@@id=deletion")
        delete_input.click()
        ...
        state_button = page.ele("tag:button@@aria-haspopup=true")
        state_button.click()

        state_select_btn = page.ele(f"tag:button@@title={dataRow["State"]}")
        state_select_btn.click()
