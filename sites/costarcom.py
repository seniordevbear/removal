...
def fill_input_data(page, dataRow) : 
    ...
    user_select.click()
    sleep(random.uniform(0.1,0.5))
    page.ele("tag:vt-option@@aria-label=Current Employee/Contractor").click()

    request_type = page.ele("tag:span@@text()= Right to Delete ")
    request_type.click()

    sender_type = page.ele("tag:span@@text()= Yes ")
    sender_type.click()
...
def costarcom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        ...
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\CostarCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://privacyportal.onetrust.com/webform/69fc85af-b1e4-4eae-966b-cf91a3cca94c/5f1a45b9-bb79-49dc-ad4d-064548192246")
