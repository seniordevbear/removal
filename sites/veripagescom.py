...
def veripagescom(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\VeripagesCom_" + fName + "-" + lName + ".png"
        ...
        profile_url = "https://veripages.com"

        if len(rows) > 0 :
            card_header = rows[0].ele("tag:div@@class=search-item-title")
            temp_str = card_header.ele("tag:span@@class=search-item-title-link").attr("data-link")
            # temp_str = temp_str.replace("/profile/", "").replace("/", "-")
            profile_url = profile_url + temp_str
            print(profile_url)

            sleep(1)
            page.get("https://veripages.com/inner/control/privacy")        
