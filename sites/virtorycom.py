...
def virtorycom(dataRow, website_name, in_user_email, run_mode) : 
    ...
    raise NotImplementedError("virtorycom: domain seized by court order (2026-09-18)")
    page = None
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\VirtoryCom_" + fName + "-" + lName + ".png"
        ...
        search_button = page.ele("tag:div@@text()=Search")
        search_button.click()
        ...
        if len(rows) > 0 :
            card_header = rows[0].ele("tag:div@@class=card-header")
            profile_url = card_header.ele("tag:a@@text()=View Profile").attr("href")
       
            sleep(1)
            page.get("https://virtory.com/prvt/control/privacy")        
            ...
            try :
                site_key = "0x4AAAAAAAB9R_fE2BDJeA6_"
                site_url = "https://virtory.com/prvt/control/privacy"
            ...
            captcha_token_input = page.ele("tag:input@@name=cf_captcha_token")
            captcha_token_input.set.attr("value", Code)
