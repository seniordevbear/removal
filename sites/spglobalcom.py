...
def fill_input_data(page, dataRow) : 
    ...
    party_element = page.ele("tag:span@@text()= I am making a request for myself ")
    party_element.click()

    country_select = page.ele("tag:input@@id=countryDSARElement")
    country_select.click()
    sleep(random.uniform(0.1,0.5))
    page.ele("tag:vt-option@@aria-label=United States").click()
    ...
    sender_type = page.ele("tag:span@@text()= Customer ")
    sender_type.click()

    request_type = page.ele("tag:span@@text()= Data Deletion ")
    request_type.click()

    request_type1 = page.ele("tag:span@@text()= Yes ")
    request_type1.click()
    ...
    company_input = page.ele("tag:input@@id=formField37DSARElement")
    company_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(company_input, "...")

    aim_input = page.ele("tag:input@@id=formField29DSARElement")
    aim_input.click()
    sleep(random.uniform(0.1,0.5))
    page.ele("tag:vt-option@@aria-label=Not Sure").click()

    detail_input = page.ele("tag:textarea@@id=requestDetailsDSARElement")
    detail_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(detail_input, "I want to remove my info.")

def spglobalcom(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name

        screenshot_save_path = screentShotDir + "\SpglobalCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://privacyportal.onetrust.com/webform/5cb57702-8ef7-437e-a62b-408fe78cd310/93391c3d-d6c8-45b1-a169-39a0b7f9fb74")
