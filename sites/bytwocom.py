...
def fill_input_data(page, dataRow) : 
    ...
    sleep(1)
    page.wait.eles_loaded("tag:span@@text()= Personal Information Deletion ")

    subject_select = page.ele("tag:input@@id=subjectTypesDSARElement")
    subject_select.click()
    sleep(random.uniform(0.1,0.5))
    page.ele("tag:vt-option@@aria-label=Consumer").click()    
    
    sleep(1)
    request_type = page.ele("tag:span@@text()= Personal Information Deletion ")
    request_type.click()
...
def bytwocom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        ...
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name 
        screenshot_save_path = screentShotDir + "\BytwoCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://privacyportal.onetrust.com/webform/e32c5e49-85c5-4969-bfa8-8cde49952e1f/6034339d-69de-41fc-829f-7b8b51d9fabe")
