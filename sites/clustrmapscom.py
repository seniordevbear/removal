...
def clustrmapscom(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        ...
        screenshot_save_path = screentShotDir + "\ClustrmapsCom_" + fName + "-" + lName + ".png"
        ...
        email_input = page.ele("tag:input@@name=inputEmail")
        email_input.click()
        ...
        try :
            site_key = "6LeYviUqAAAAAPB4MWAOTWseCsrro_O7DIxMXW31"
            site_url = "https://clustrmaps.com/bl/opt-out"
        ...
        next_btn = page.ele("tag:button@@text()=Next Step")
        next_btn.remove_attr("disabled")
        next_btn.click()

        sleep(1)

        fullName_input = page.ele("tag:input@@id=personName")
        ...
        city_state_input = page.ele("tag:input@@id=cityState")
        city_state_input.click()
        sleep(random.uniform(0.1, 0.5))
        _human_type2(city_state_input, city_state)


        item_element = page.ele("tag:div@@class=deep-suggestion-block")
        print(item_element)
        page.run_js("arguments[0].click();", item_element)

        find_btn = page.ele("tag:button@@text()=Find Person")
        find_btn.click()

        sleep(1)

        proceed_btns = page.eles("tag:a@@text()=Proceed to Opt Out")

        if len(proceed_btns) > 0 :
            proceed_btns[0].click()
            sleep(1)
            form_container = page.ele("tag:form@@class=deletor")
            checkboxes = form_container.eles("tag:input@@type=checkbox")
            for item in checkboxes :
                item.click()
                sleep(0.5)
            
            apply_btn = page.ele("tag:input@@value=Apply")
            apply_btn.click()
