...
def publicdatacheckcom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\PublicDataCheckCom_" + fName + "-" + lName + ".png"
        ...
        try:
            ...
            result = solver.turnstile(sitekey=SITE_KEY, url="https://www.publicdatacheck.com/help-center/privacy-requests")
            Code=result['code']
