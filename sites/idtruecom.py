...
def idtruecom(dataRow, website_name, in_user_email, run_mode) : 
    ...
    try : 
        ...
        screenshot_save_path = screentShotDir + "\IDTrueCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://www.idtrue.com/optout")

        sleep(random.uniform(3, 5))

        fName_input = page.ele("tag:input@@id=o_first_name")
        fName_input.click()
        print("typing the first name...")
        sleep(random.uniform(0.1,0.5))
        _human_type2(fName_input, fName)

        lName_input = page.ele("tag:input@@id=o_last_name")
        lName_input.click()
        print("typing the last name...")
        sleep(random.uniform(0.1,0.5))
        _human_type2(lName_input, lName)

        city_input = page.ele("tag:input@@id=o_city")
        city_input.click()
        ...
        if len(rows) > 0 : 
            view_btn = rows[0].ele("tag:button")
            view_btn.click()
            ...
            confirm_checkbox = page.ele("tag:input@@id=confirm_check_box")
            ...
            opt_out_btn = page.ele("tag:button@@type=submit")
            opt_out_btn.click()
