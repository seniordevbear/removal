...
def fill_input_data(page, dataRow) : 
    ...
    phone_input = page.ele("tag:input@@name=phoneOptOut")
    phone_input.click()
    ...
    birth_day_select = page.ele("tag:select@@name=dob_day")
    birth_day_select.select.by_text(str(dataRow["Birth Day"]))

    birth_month_select = page.ele("tag:select@@name=dob_month")
    birth_month_select.select.by_text(str(dataRow["Birth Month"]))

    birth_year_select = page.ele("tag:select@@name=dob_year")
...
def freebackgroundcheckio(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        ...
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\FreeBackgroundCheckIO_" + fName + "-" + lName + ".png"
        ...
        page.get("https://freebackgroundcheck.io/optout")
        ...
        try :
            ...
            site_url = "https://freebackgroundcheck.io/optout"
