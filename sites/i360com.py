...
def fill_input_data(page, dataRow) : 
    ...
    request_type = page.ele("tag:span@@text()= Erase personal data ")
    request_type.click()

    state_select = page.ele("tag:input@@id=formField40DSARElement")
    state_select.click()
    ...
    address_input = page.ele("tag:input@@id=formField34DSARElement")
    address_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(address_input, dataRow["Address"])

    city_input = page.ele("tag:input@@id=formField36DSARElement")
    city_input.click()
    sleep(random.uniform(0.1,0.5))
...
def i360com(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        ...
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\I360Com_" + fName + "-" + lName + ".png"
        ...
        page.get("https://privacyportal.onetrust.com/webform/77dff651-9f08-40cd-99fe-a7c487b2504d/fd25e566-5931-4987-ae94-13ffd3306913")
