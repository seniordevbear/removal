...
def fill_input_data(page, dataRow) : 
    ...
    street_input = page.ele("tag:input@@name=street")
    street_input.click()
    sleep(random.uniform(0.1,0.5))
...
def ncsolutionscom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + r"\NSolutionsCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://ncsolutions.com/do-not-sell-my-information/")

        fill_input_data(page, dataRow)

        sleep(random.uniform(3, 5))
        page.wait.ele_displayed("tag:button@@text()=Submit")
