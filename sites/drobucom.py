...
def fill_input_data(page, dataRow) : 
    ...
    street_input = page.ele("tag:input@@name=address")
    street_input.clear()
    street_input.click()
    sleep(random.uniform(0.1,0.5))
    ...
    zip_input = page.ele("tag:input@@name=zip_code")
    zip_input.clear()
    zip_input.click()
    sleep(random.uniform(0.1,0.5))
    ...
    radio_element = page.ele("tag:input@@id=id_do_not_sell_0")
    radio_element.click()
    sleep(random.uniform(0.5,1))

    iframe_container = page.ele("tag:iframe@@title=reCAPTCHA")
    print(iframe_container.attr('name'))
    g_recaptcha_response_input = iframe_container.ele("tag:input@@id=recaptcha-token")
    print(g_recaptcha_response_input)

    form_container = page.ele("tag:form@@id=legal_opt_out_form")
    print(form_container)
    form_container.run_js("this.submit();")    

def drobucom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        ...
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\DrobuCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://drobu.jangl.com/legal/opt-out/")
