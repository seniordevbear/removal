...
def fill_input_data(page, dataRow) : 
    ...
    lName_input = page.ele("tag:input@@id=o_last")
    lName_input.click()
    ...
    email_input = page.ele("tag:input@@id=o_email")
    email_input.click()
    print("typing the email...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(email_input, email_str)

    yes_checkbox = page.ele("tag:input@@id=o_terms")
    yes_checkbox.click()

def searchpeoplefreecom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\SearchPeopleFreeCom_" + fName + "-" + lName + ".png"
