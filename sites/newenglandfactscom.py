...
def newenglandfactscom(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + r"\NewEnglandFactsCom_" + fName + "-" + lName + ".png"
        ...
        if len(people_list) > 0 :
            ...
            print(profile_url)
            page.get("https://newenglandfacts.com/ng/control/privacy")
            sleep(2)
