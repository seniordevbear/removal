...
def fill_input_data(page, dataRow) : 
    ...
    page.wait.eles_loaded("tag:input@@name=acceptance1")
    check_input_1 = page.ele("tag:input@@name=acceptance1")
    check_input_1.click()

    page.wait.eles_loaded("tag:input@@name=acceptance2")
    check_input_2 = page.ele("tag:input@@name=acceptance2")
    check_input_2.click()
    ...
    email_input = page.ele("tag:input@@name=your-email")
    email_input.click()
...
def lakeb2bcom(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\Lakeb2bCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://www.lakeb2b.com/do-not-sell-my-data")

        fill_input_data(page, dataRow)

        sleep(random.uniform(3, 5))
        page.wait.ele_displayed("tag:input@@value=Get a Quote Now!")
        submit_button = page.ele("tag:input@@value=Get a Quote Now!")
        submit_button.click()

        page.wait.ele_displayed("tag:h1@@text()=Thank you for contacting us.")
