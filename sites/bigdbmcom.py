...
def fill_input_data(page, dataRow) : 
    ...
    checkbox_element = page.ele("tag:input@@id=formCheckAll")
    checkbox_element.click()

    fName_input = page.ele("tag:input@@id=horizontal-firstname-input")
    fName_input.clear()
    fName_input.click()
    ...
    lName_input = page.ele("tag:input@@id=horizontal-lastname-input")
    lName_input.clear()
    lName_input.click()
    print("typing the first name...")
    sleep(random.uniform(0.1,0.5))
    ...
    email_input = page.ele("tag:input@@id=horizontal-email-input")
    email_input.clear()
    email_input.click()
    ...
    phone_input = page.ele("tag:input@@id=horizontal-phone-input")
    phone_input.clear()
    phone_input.click()
    sleep(random.uniform(0.1,0.5))
    ...
    address_input = page.ele("tag:input@@id=horizontal-address1-input")
    address_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(address_input, dataRow["Address"])

    street_input = page.ele("tag:input@@id=horizontal-address2-input")
    street_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(street_input, dataRow["Street"])


    city_input = page.ele("tag:input@@id=horizontal-city-input")
    city_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(city_input, dataRow["City"])

    state_element = page.ele("tag:select@@id=horizontal-state-input")
    state_element.select.by_text(usaStateDictionary[dataRow["State"]])

    zip_input = page.ele("tag:input@@id=horizontal-zip-input")
    zip_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(zip_input, str(dataRow["Zipcode"]))

    submit_checkbox = page.ele("tag:input@@id=inlineFormCheck")
    submit_checkbox.set.attr("checked", True)
    
def bigdbmcom(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        ...
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\BigdbmCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://optout.bigdbm.com/")
        ...
        try :
            site_key = "6Lf9buopAAAAANjCscXjZwftN1LkMbXV10OjAaOQ"
            site_url = "https://optout.bigdbm.com/"
        ...
        submit_button = page.ele("tag:button@@id=main-submit-button")
        submit_button.click()
