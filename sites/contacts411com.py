...
def fill_input_data(page, dataRow) : 
    ...
    address_input = page.ele("tag:input@@name=address1")
    address_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(address_input, dataRow["Address"])

    street_input = page.ele("tag:input@@name=address2")
    street_input.click()
    sleep(random.uniform(0.1,0.5))
    ...
    email_confirm_input = page.ele("tag:input@@name=confirmationEmail")
    ...
    try :
        site_key = "6LdDD9QqAAAAAKbQJCNvWP4QwJToGJCNvOHzp5lu"
        site_url = page.url
...
def contacts411com(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        ...
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\Contacts411Com_" + fName + "-" + lName + ".png"
        ...
        page.get("https://contacts411.com/opt-out")
