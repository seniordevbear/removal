...
def freepeopledirectorycom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\FreePeopleDirectoryCom_" + fName + "-" + lName + ".png"
        ...
        if search_result_div != None :
            ...
            if len(rows) > 0 :
                profile_url = rows[0].ele("tag:a@@class:font-display").attr("href")
                print(profile_url)
                sleep(1)
