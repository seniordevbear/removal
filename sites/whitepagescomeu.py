...
def whitepagescomeu(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        ...
        error_path = os.path.join(screentShotDir, f"WhitePagesComEU_{fName}-{lName}_error.txt")
        try:
            with open(error_path, "w", encoding="utf-8") as f:
                f.write(f"Error while processing whitepages.com (EU) removal for {dataRow.get('Name', '')}: {e}\n")
        except Exception:
    ...
    success_path = os.path.join(screentShotDir, f"WhitePagesComEU_{fName}-{lName}.txt")
    try:
        with open(success_path, "w", encoding="utf-8") as f:
            f.write(f"Removal request email sent to whitepages.com (EU) for {dataRow.get('Name', '')}\n")
