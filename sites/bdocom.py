...
def fill_input_data(page, dataRow) : 
    ...
    fName_input = page.ele("tag:input@@id=form_005d_fld_0")
    fName_input.click()
    print("typing the first name...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(fName_input, fName)

    lName_input = page.ele("tag:input@@id=form_005d_fld_1")
    lName_input.click()
    print("typing the last name...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(lName_input, lName)

    email_input = page.ele("tag:input@@id=form_005d_fld_2")
    email_input.click()
    print("typing the email...")
    sleep(random.uniform(0.1,0.5))
    _human_type2(email_input, generate_email(dataRow["Name"]))

    zipcode_input = page.ele("tag:input@@id=form_005d_fld_3")
    zipcode_input.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(zipcode_input, str(dataRow["Zipcode"]))

    office_select = page.ele("tag:select@@id=form_005d_fld_4")
    office_select.select.by_text("Darwin")

    message_textarea = page.ele("tag:textarea@@id=form_005d_fld_7")
    message_textarea.click()
    sleep(random.uniform(0.1,0.5))
    _human_type2(message_textarea, "I want to remove my info from your site.")

    agree_checkbox = page.ele("tag:label@@for=form_005d_fld_10-0")
    ...
    try :
        site_key = "6LctqAsUAAAAAK3jaxOkexQ-5vz4zpy0oMlFhWOv"
        site_url = page.url
    ...
    submit_button = page.ele("tag:input@@id=form_005d_ao_submit_input")
    submit_button.run_js("this.click();")


def bdocom(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name 
        screenshot_save_path = screentShotDir + "\BdoCom_" + fName + "-" + lName + ".png"
        ...
        # page.load_mode=None
        page.get("https://www.bdo.com.au/en-au/content/forms/opt-out-your-details")
