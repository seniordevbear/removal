...
def persontrustcom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\PersonTrustCom_" + fName + "-" + lName + ".png"
        ...
        page.get("https://persontrust.com/")
        ...
        profile_url = "https://persontrust.com/#profile-"

        if len(rows) > 0 :
            profile_url_temp = rows[0].ele("tag:a@@text()=View Profile").attr("onclick")
            match_1 = re.search(r"pid:'(.*?)'", profile_url_temp)
            if match_1 :
                pid = match_1.group(1)
                profile_url = profile_url + pid + "-" + fName + "-" + lName
                sleep(1)
                page.get("https://persontrust.com/ng/control/privacy")        
                sleep(1)
