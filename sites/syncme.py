...
def fill_input_data(page, dataRow) : 
    ...
    phone_input = page.ele("tag:input@@class=RemoveInfoSection_inputNumber__mainInput___7UbBk")
    phone_input.click()

    sleep(random.uniform(0.1,0.5))
    _human_type2(phone_input, generate_phone_number())

    country_button = page.ele("tag:button@@class:SearchNumber_searchNumber__countrySelect")
    country_button.click()

    country_container_div = page.ele("tag:div@@class:SearchNumber_countries__containe")
    country_list_div = country_container_div.eles("tag:div@@class:SearchNumber_countries__item")
    for country in country_list_div :
        if country.text == "United States +1" :
            country.click()
            break
def syncme(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\SyncMe_" + fName + "-" + lName + ".png"
        ...
        page.get("https://sync.me/unsubscribe/")
