...
def weinformorgeu(dataRow, website_name, in_user_email, run_mode) : 
    page = None
    try : 
        ...
        error_path = os.path.join(screentShotDir, f"WeInformOrgEU_{fName}-{lName}_error.txt")
        try:
            with open(error_path, "w", encoding="utf-8") as f:
                f.write(f"Error while processing weinform.org (EU) removal for {dataRow.get('Name', '')}: {e}\n")
        except Exception:
    ...
    success_path = os.path.join(screentShotDir, f"WeInformOrgEU_{fName}-{lName}.txt")
    try:
        with open(success_path, "w", encoding="utf-8") as f:
            f.write(f"Removal request email sent to weinform.org (EU) for {dataRow.get('Name', '')}\n")
