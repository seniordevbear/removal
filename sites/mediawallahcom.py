...
def fill_input_data(page, dataRow) : 
    ...
    request_type = iframe_container.ele("tag:span@@text()= Data Deletion ")
    request_type.click()
    ...
    state_select.click()
    sleep(random.uniform(0.1,0.5))
    iframe_container.ele(f"tag:vt-option@@text():{dataRow["State"]}").click()
...
def mediawallahcom(dataRow, website_name, in_user_email, run_mode) : 
    try : 
        fName = dataRow["Name"].split()[0] # split string based on space to get first name
        lName = dataRow["Name"].split()[-1]# split string based on space to get last name
        screenshot_save_path = screentShotDir + "\MediaWallahCom_" + fName + "-" + lName + ".png"
