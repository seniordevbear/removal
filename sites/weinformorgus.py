...
def weinformorgus(dataRow, website_name, in_user_email, run_mode) : 
    ...
    try : 
        ...
        COMPANY_NAME = "weinform.org"
        PROFILE_URL = ""
        TO_EMAIL = "support@weinform.org"
        ...
        # Even on error, generate a simple text artifact so callers have a path to upload.
        error_path = os.path.join(screentShotDir, f"WeInformOrg_{fName}-{lName}_error.txt")
        try:
            with open(error_path, "w", encoding="utf-8") as f:
                f.write(f"Error while processing weinform.org removal for {dataRow.get('Name', '')}: {e}\n")
        except Exception:
            error_path = screentShotDir
        try :
    ...
    # Create a small proof file indicating the email request was sent successfully.
    success_path = os.path.join(screentShotDir, f"WeInformOrg_{fName}-{lName}.txt")
    try:
        with open(success_path, "w", encoding="utf-8") as f:
            f.write(f"Removal request email sent to weinform.org for {dataRow.get('Name', '')}\n")
